...
public class QueryTemplates {
    public static final String ACCOUNT_SUMMARY = "select A.project_id as project_id,totalSize,totalVolumes,"
            ...
            + " coalesce(Q.volumeLimit,(select qc.hard_limit from quota_classes qc where qc.resource='volumes' and qc.deleted=0)) as volumeLimit,"
            + " coalesce(Q.gbLimit,(select qc.hard_limit from quota_classes qc where qc.resource='gigabytes' and qc.deleted=0)) as gbLimit,"
            + " coalesce(Q.snapshotLimit,(select qc.hard_limit from quota_classes qc where qc.resource='snapshots' and qc.deleted=0)) as snapshotLimit,"
            + " coalesce(Q.perVolumeGBLimit,(select qc.hard_limit from quota_classes qc where qc.resource='per_volume_gigabytes' and qc.deleted=0)) as perVolumeGBLimit,"
            ...
            + " from quotas group by project_id) Q on A.project_id = Q.project_id left join"
    ...
    public static final String ACCOUNT_SUMMARY_RESULTS = "select count(AC.project_id) as totalResults from "
            ...
            + " from quotas group by project_id) Q on A.project_id = Q.project_id left join"
            + " (select count(b.id) as totalSnapshots,b.project_id from backups b where b.deleted = 0 and b.project_id IS NOT NULL group by b.project_id)"
            + " B on A.project_id = B.project_id where %s like ?1) AC";
